...
class DefiningPrims extends Primitives {
  ...
  public Object dispatch(int offset, Object[] args, LContext lc){
    ...
    case 15: return prim_procs(lc);
    case 16: return prim_prims(lc);
    case 17: return prim_erprocs(lc);
    case 18: lc.oktoredefine = Logo.aBoolean(args[0], lc); return null;
  ...
  Object prim_procs(LContext lc){
    Hashtable syms = lc.oblist;
    Vector<Object> vec = new Vector<Object>();
    for (Enumeration en = syms.elements(); en.hasMoreElements() ;){
      Symbol sym = (Symbol)(en.nextElement());
      if((sym.fcn!=null)&&(sym.fcn.instance instanceof Ufun)) vec.add(sym);;
    }
    Object[] result = new Object[vec.size()];
    vec.copyInto(result);
    return result;
  ...
  Object prim_erprocs(LContext lc){
    Hashtable syms = lc.oblist;
    for (Enumeration en = syms.elements(); en.hasMoreElements() ;){
      Symbol sym = (Symbol)(en.nextElement());
      if((sym.fcn!=null)&&(sym.fcn.instance instanceof Ufun)) sym.fcn=null;
    }
    return null;
  }
